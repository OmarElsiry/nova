...
-- Enable RLS on wallet_connections (only if not already enabled)
DO $$ BEGIN
  IF NOT EXISTS (
...
DROP POLICY IF EXISTS "Users can manage their own wallet connections" ON public.wallet_connections;
...
-- Create trigger function for wallet_connections updated_at
